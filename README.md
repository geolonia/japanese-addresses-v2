...
以前、「[Geolonia 住所データ](https://github.com/geolonia/japanese-addresses)」を管理していましたが、v2は従来版と比べて下記の違いがあります。

* 住居表示住所データに対応（番地・号までのデータが含まれる）
* 地番住所データに対応（住居表示住所が導入されていない地域のデータが含まれる）
...
* アドレス・ベース・レジストリの整備状況によって、住居表示が実施されている町字でも住居表示住所のデータが無い場合や、地番住所のデータが無い場合があります。また、住居表示や地番の文字列が存在しても位置情報データがまだ存在しないケースもあります。
...
本データは、「アドレス・ベース・レジストリ」（デジタル庁）(https://catalog.registries.digital.go.jp) をもとに株式会社 Geolonia が作成したものです。

## 貢献方法

* 本データ・ツールに不具合がある場合には、[Issue](https://github.com/geolonia/japanese-addresses-v2/issues) または[プルリクエスト](https://github.com/geolonia/japanese-addresses-v2/pulls)にてご報告ください。
